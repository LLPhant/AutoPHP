...
class AutoPHP
{
    ...
    public function __construct(
        ...
        public OutputAgentInterface $outputAgent = new AutoPHPCLIOutputUtils(),
    ) {
    ...
    private function getObjectiveResult(): ?string
    {
        ...
        $functionCalled  = $stringOrFunctionInfo[0];


        if (! $functionCalled instanceof FunctionInfo) {
            // Shouldn't be null as OPENAI should call the function
            return null;
        }

        $objectiveData = FunctionRunner::run($functionCalled);
